...
export function ButtonForm({ children, active = true, loadAnimation = true }: { children: ReactNode, active?: boolean, loadAnimation?: boolean }) {
    ...
    return loadAnimation && clicked ? (
        <Div.buttonComponentStyle>
            <button id="buttonComponent" type="submit" disabled={!active || clicked} className="stage"><Loading /></button>
        </Div.buttonComponentStyle>
    ) : (
        <Div.buttonComponentStyle>
            <button id="buttonComponent" type="submit" disabled={!active || clicked} onClick={() => { setClicked(true) }}>{children}</button>
        </Div.buttonComponentStyle>
    )
}